...
function makePatchTestEnvironmentFromReservedToAvailable(
  ...
  return async function patchTestEnvironmentFromReservedToAvailable(
    ...
    loggingTags.actionParameters = {
      TestEnvironmentName: retrievedTestEnvironment.name,
    };
    ...
    if (httpServer.emitter) {
      httpServer.emitter.emit(
        ...
        `test environment updated ${retrievedTestEnvironment.name}`,
      );
    }
    ...
    logger.info(
      loggingTags,
      `PATCH request to update Test Environment object status from Reserved to Available \
