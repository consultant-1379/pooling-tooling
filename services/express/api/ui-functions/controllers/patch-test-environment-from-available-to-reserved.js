...
function makePatchTestEnvironmentFromAvailableToReserved(
  dbOperator,
  ...
  return async function patchTestEnvironmentFromAvailableToReserved(httpRequest) {
    ...
    const loggingTags = {
      ...
      action: 'patchTestEnvironmentFromAvailableToReserved',
      actionParameters: {},
    };
    ...
    if (checkRequestIdExists(retrievedTestEnvironment)) {
      const requestDataToEdit = {
        lastReservedAt: new Date().toUTCString(),
        status: 'Unreserved',
      };
    ...
    logger.info(loggingTags, `PATCH request to update Test Environment object status from Available to Reserved \
...
module.exports = { makePatchTestEnvironmentFromAvailableToReserved };
