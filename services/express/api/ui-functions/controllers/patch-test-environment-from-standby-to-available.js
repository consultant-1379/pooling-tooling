...
function makePatchTestEnvironmentFromStandbyToAvailable(
  ...
  return async function patchTestEnvironmentFromStandbyToAvailable(httpRequest) {
    ...
    if (httpServer.emitter) {
      httpServer.emitter.emit('testEnvironmentEvent', `test environment updated ${retrievedTestEnvironment.name}`);
    }

    loggingTags.res = logger.logFormatter([updatedTestEnvironment]);
    logger.info(loggingTags, `PATCH request to update Test Environment object status from Standby to Available \
with name:${retrievedTestEnvironment.name}.`);
