...
describe('Unit Test: (UI Functions service) Testing patch controller to update the Test Environment (Quarantine -> Reserved) and/or Request', () => {
  const emitter = { emit: () => { } };
  it('should return updatedTestEnvironment only.', async () => {
    ...
    const testEnvironmentChanges = await patchTestEnvironmentFromQuarantinedToReserved(fakeHttpRequest);
  ...
  it('throws an error when retrieved test environment is an empty object', async () => {
    ...
    await patchTestEnvironmentFromQuarantinedToReserved(fakeHttpRequest).catch((error) => {
