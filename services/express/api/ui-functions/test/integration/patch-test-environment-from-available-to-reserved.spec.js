...
const { makePatchTestEnvironmentFromAvailableToReserved } = require('../../controllers/patch-test-environment-from-available-to-reserved');
...
describe('Integration Test: (UI Functions service) Testing patch controller to update the Test Environment (Available -> Reserved) and/or Request', () => {
  beforeEach(async () => {
  ...
  it('should return updatedTestEnvironment only.', async () => {
    ...
    expect(testEnvironmentFromDbBeforePatching.status).toEqual('Available');
    ...
    const testEnvironmentChanges = await patchTestEnvironmentFromAvailableToReserved(testHttpRequest);
  ...
  it('should throw an error if the test environment is undefined or object is empty.', async () => {
    ...
    await patchTestEnvironmentFromAvailableToReserved(testHttpRequest).catch((error) => {
  ...
  it('should throw an \'Invalid http request to update Test Environment and/or Request entity.\'', async () => {
    const fakeTestEnvironment = makeFakeTestEnvironment({ status: 'Available' });
    await dbOperator.insert(fakeTestEnvironment, 'testEnvironments');
