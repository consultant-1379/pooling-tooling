...
const { makePatchTestEnvironmentFromStandbyToRefreshing } = require('../../controllers/patch-test-environment-from-standby-to-refreshing');
...
describe('Integration Test: (UI Functions service) Testing patch controller to update the Test Environment (Standby -> Refreshing) and/or Request', () => {
  beforeEach(async () => {
  ...
  it('should return updatedTestEnvironment only.', async () => {
    ...
    const testEnvironmentChanges = await patchTestEnvironmentFromStandbyToRefreshing(testHttpRequest);
    expect(testEnvironmentChanges.requestId).toEqual('');
    expect(testEnvironmentChanges.status).toEqual('Refreshing');
    ...
    expect(testEnvironmentFromDbAfterPatching.status).toEqual('Refreshing');
  ...
  it('should throw an error if the test environment is undefined or object is empty.', async () => {
    ...
    await patchTestEnvironmentFromStandbyToRefreshing(testHttpRequest).catch((error) => {
